...
const Schema = class {
    ...
    for (let schemaKey of schemaKeys) {
      ...
      if (typeof schemaData !== "string" && !schemaData.type)
        error = this.#setError(schemaKey, error, {
          // error handling
          type: ` "${schemaKey}" value is not a supported ${
            schemaData.type ?? "datatype"
          }`,
        });

      // set key to object if key value is type
      if (typeof schemaData === "string") schemaData = { type: schemaData };

      // check if the schema key exist in the data
      const valueExist = Object.keys(objectData).includes(schemaKey);
      const isTypeSupported = this.supportedType.includes(
        schemaData.type?.toLowerCase()
      );
      ...
      if (schemaData.type === "object" && schemaData.$_data && dataValue) {
      ...
      if (schemaData.type === "array" && schemaData.$_data) {
        if (!Array.isArray(schemaData.$_data))
          error = this.#setError(schemaKey, error, {
            // error handling
            type: ` Schema key: "${schemaKey}" $_data is not a supported ${
              schemaData.type ?? "datatype"
            }`,
          });
      ...
      if (schemaData.required && !objectData[schemaKey])
        error = this.#setError(schemaKey, error, {
          // error handling
          required: `"${schemaKey}" field is required!`,
        });
      ...
      if (schemaData.enum && !Array.isArray(schemaData.enum))
        ...
        schemaData.enum &&
        !schemaData.enum.includes(objectData[schemaKey])
      )
        error = this.#setError(schemaKey, error, {
          // error handling
  ...
  #typeValidation(type, value) {
    ...
    else if (type === "date") return this.#isDate(value);
    else if (type === "float") return this.#isFloat(value);
    // check if value is a valid array
    else if (type === "array") return Array.isArray(value);
    // check if value is a valid email
    else if (type === "email") return validator.isEmail(value);
    //  check if value is a jwt
    else if (type === "jwt") return validator.isJWT(value);
